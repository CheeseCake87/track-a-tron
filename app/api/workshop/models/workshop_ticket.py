...
class WorkshopTicket(db.Model):
    ...
    # Data
    workshop_tag = s.Column(s.String, nullable=True)  # Unique identifier
    ...
    # Relationships
    rel_added_by = s.orm.relationship("SystemUser", foreign_keys=[fk_user_id], viewonly=True)
    rel_assigned_to = s.orm.relationship(
        "SystemUser", foreign_keys=[fk_assigned_user_id],
        viewonly=True
    )
    rel_client = s.orm.relationship("Client", viewonly=True)
    rel_devices = s.orm.relationship("WorkshopTicketDevice", viewonly=True)
    rel_items = s.orm.relationship("WorkshopTicketItem", viewonly=True)
    rel_notes = s.orm.relationship(
        "WorkshopTicketNote", order_by="WorkshopTicketNote.created.desc()",
        viewonly=True
    )
